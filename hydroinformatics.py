...
class StreamReach:
    def __init__(self, latitude, longitude, data_source, geoglows_streams=None, nwm_ds=None):
    ...
    def _load_geoglows(self):
        ...
        bbox_coords = make_bbox(self.latitude, self.longitude, 0.003)
    ...
    def _load_usgs(self):
        ...
        bbox = make_bbox(self.latitude, self.longitude, 0.05)
        ...
        # print(nearest_site[['site_no', 'station_nm', 'distance_km']])
...
def compare_hydrographs(reaches):
    ...
    if "USGS" in data_sources:
        plt.title(f"Hydrograph Comparison for {name}")
    else:
...
def compare_fdcs(reaches):
    ...
    if "USGS" in data_sources:
        plt.title(f"Flow Duration Curve Comparison for {name}")
    else:
...
def main(lhd_csv_path, streams_gpkg_path, ):
    ...
    for index, row_i in lhd_df.iterrows():
        ...
        for source in sources:
            try:
                ...
                reach = StreamReach(lat, lon, source,
    ...
    plot_discharge_comparisons(results_df)
